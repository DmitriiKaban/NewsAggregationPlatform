...
export default function App() {
            ...
            try {
                ...
                if (tg.initData && tg.initData.length > 0) {
                    ...
                    if (userJson) {
                        ...
                        setUser({ id: parsedUser.id, first_name: parsedUser.first_name || 'User', username: parsedUser.username });
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            fetch(`${apiBaseUrl}/analytics/insights`, fetchOptions).then(r => r.json()).catch(() => ({ topSources: [], dauStats: [] })),
        ])
            .then(([profileData, recommendationsData, insightsData]) => {
                ...
                if (profileData.language && ['en', 'ro', 'ru'].includes(profileData.language)) {
                    setLang(profileData.language as Language);
                }
    ...
    useEffect(() => {
        ...
        const handleBack = () => {
            if (isEditingInterests) { setIsEditingInterests(false); setInterests(originalInterests); }
            else { setActiveTab('interests'); }
        };
        ...
        try {
            ...
            if (profileRes.ok) {
                ...
                if (Array.isArray(data.sources)) {
                    setSources(data.sources.map((s: any) => ({ id: s.id, url: s.url || s.name || '', isReadAll: s.isReadAll || false })).filter((s: Source) => s.url));
                }
    ...
    if (!user) return (
        <div style={{ minHeight: '100vh', background: colors.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: '-apple-system, sans-serif' }}>
            <div style={{ textAlign: 'center', maxWidth: '400px', color: colors.text }}>
                <div style={{ fontSize: '64px', marginBottom: '24px' }}>❌</div>
                <h2 style={{ fontSize: '24px', fontWeight: '600', margin: '0 0 12px 0' }}>{tr('error.user_not_found', lang)}</h2>
                <p style={{ fontSize: '15px', color: colors.hint, lineHeight: 1.5, margin: '0 0 20px 0' }}>{tr('error.open_from_telegram', lang)}</p>
                {error && (
                    <div style={{ padding: '12px', background: `${colors.hint}20`, borderRadius: '8px', fontSize: '13px', color: colors.hint, textAlign: 'left' }}>
    ...
    // ── Main UI ───────────────────────────────────────────────────────────────
    return (
        <div style={{ minHeight: '100vh', background: colors.bg, color: colors.text, fontFamily: '-apple-system, sans-serif', paddingBottom: '20px' }}>
            ...
            <div style={{ padding: '20px' }}>
                ...
                {activeTab === 'interests' && (
                    <div>
                        <label style={{ display: 'block', fontSize: '15px', fontWeight: '600', marginBottom: '12px', color: colors.text }}>
                            {tr('interests.title', lang)}
                        </label>

                        {!isEditingInterests ? (
                            <>
                                <div style={{ padding: '16px', background: colors.secondaryBg, borderRadius: '12px', border: `1px solid ${colors.hint}20`, minHeight: '80px', fontSize: '15px' }}>
                                    {originalInterests || <span style={{ color: colors.hint, fontStyle: 'italic' }}>{tr('interests.placeholder', lang)}</span>}
                                </div>

                                {/* Mode indicator */}
                                <div style={{ marginTop: '12px', padding: '10px 14px', background: strictMode ? `${colors.danger}15` : `${colors.button}15`, borderRadius: '10px', border: `1px solid ${strictMode ? colors.danger : colors.button}30` }}>
                                    <div style={{ fontSize: '13px', fontWeight: '600', color: strictMode ? colors.danger : colors.button }}>
                                ...
                                <button onClick={() => setIsEditingInterests(true)} style={{ marginTop: '16px', padding: '14px 24px', background: colors.button, color: colors.buttonText, border: 'none', borderRadius: '10px', fontSize: '15px', fontWeight: '600', width: '100%', cursor: 'pointer' }}>
                                    {tr('interests.button_update', lang)}
                                </button>
                            </>
                        ) : (
                            <>
                                <textarea
                                    ...
                                    autoFocus
                                    style={{ width: '100%', minHeight: '120px', padding: '14px', fontSize: '15px', border: `2px solid ${colors.button}`, borderRadius: '12px', background: colors.bg, color: colors.text, boxSizing: 'border-box', resize: 'vertical' }}
                                />
                                <p style={{ fontSize: '13px', color: colors.hint, marginTop: '8px' }}>{tr('interests.hint', lang)}</p>
                            </>
                        )}
                        ...
                        {!isEditingInterests && recommendations.length > 0 && (
                            <div style={{ marginTop: '32px' }}>
                                <h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '4px' }}>{tr('recommendations.title', lang)}</h3>
                                <p style={{ fontSize: '13px', color: colors.hint, marginBottom: '12px' }}>{tr('recommendations.subtitle', lang)}</p>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    {recommendations.map(rec => (
                                        <div key={rec.url} style={{ padding: '12px 16px', background: colors.secondaryBg, borderRadius: '12px', border: `1px solid ${colors.button}40`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <div>
                                            ...
                                            <button onClick={() => handleAddSource(rec.url)} style={{ background: colors.button, color: colors.buttonText, border: 'none', padding: '6px 14px', borderRadius: '16px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
                ...
                {activeTab === 'sources' && (
                    <div>
                        {/* Strict mode toggle */}
                        <div style={{ background: colors.secondaryBg, padding: '16px', borderRadius: '12px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: `1px solid ${colors.hint}40` }}>
                            <div>
                                <h4 style={{ margin: '0 0 4px 0', fontSize: '15px' }}>{tr('sources.strict_mode', lang)}</h4>
                                <span style={{ fontSize: '13px', color: colors.hint }}>{tr('sources.strict_mode_desc', lang)}</span>
                            </div>
                            <div onClick={toggleStrictMode} style={{ width: '46px', height: '26px', background: strictMode ? colors.success : colors.hint, borderRadius: '13px', position: 'relative', cursor: 'pointer' }}>
                                <div style={{ width: '22px', height: '22px', background: '#fff', borderRadius: '50%', position: 'absolute', top: '2px', left: strictMode ? '22px' : '2px', transition: 'left 0.3s' }} />
                            </div>
                        </div>

                        {/* Source list */}
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            <h3 style={{ margin: 0, fontSize: '17px', fontWeight: '600' }}>{tr('sources.title', lang)}</h3>
                            <button onClick={() => handleAddSource()} style={{ padding: '8px 16px', background: colors.button, color: colors.buttonText, border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer' }}>
                                {tr('sources.add_button', lang)}
                            </button>
                        </div>

                        {sources.length === 0 ? (
                            <div style={{ textAlign: 'center', color: colors.hint, padding: '32px 0' }}>{tr('sources.empty', lang)}</div>
                        ) : (
                                ...
                                {sources.map((source, i) => (
                                    <div key={source.id} style={{ padding: '14px', background: colors.secondaryBg, borderRadius: '12px', border: `1px solid ${colors.hint}20` }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                                            <span style={{ fontSize: '15px', wordBreak: 'break-all', fontWeight: '500' }}>📡 {source.url}</span>
                                            <button onClick={() => handleRemoveSource(source.url, i)} style={{ background: 'transparent', border: 'none', color: colors.danger, fontSize: '18px', fontWeight: 'bold', cursor: 'pointer' }}>✕</button>
                                        </div>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: `1px solid ${colors.hint}20`, paddingTop: '12px' }}>
                                            <span style={{ fontSize: '14px', color: colors.hint }}>{tr('sources.bypass_ai', lang)}</span>
                                            <div onClick={() => toggleReadAll(source.id, i)} style={{ width: '40px', height: '22px', background: source.isReadAll ? colors.button : colors.hint, borderRadius: '11px', position: 'relative', cursor: 'pointer' }}>
                                                <div style={{ width: '18px', height: '18px', background: '#fff', borderRadius: '50%', position: 'absolute', top: '2px', left: source.isReadAll ? '20px' : '2px', transition: 'left 0.3s' }} />
                                            </div>
                                        </div>
                                    </div>
                                ))}
                ...
                {activeTab === 'insights' && (
                    <div>
                        <h3 style={{ fontSize: '18px', fontWeight: '700', marginBottom: '16px' }}>{tr('insights.title', lang)}</h3>

                        {/* DAU chart */}
                        <div style={{ background: colors.secondaryBg, padding: '20px 16px', borderRadius: '16px', marginBottom: '24px', border: `1px solid ${colors.hint}20` }}>
                            <h4 style={{ margin: '0 0 4px 0', fontSize: '15px' }}>{tr('insights.dau_title', lang)}</h4>
                            <p style={{ margin: '0 0 20px 0', fontSize: '12px', color: colors.hint }}>{tr('insights.dau_desc', lang)}</p>
                                ...
                                {dauStats.length > 0 ? dauStats.map((d, index) => {
                                    ...
                                    return (
                                        <div key={index} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
                                            <span style={{ fontSize: '11px', fontWeight: 'bold', marginBottom: '4px', color: colors.text }}>{d.count}</span>
                                            <div style={{ width: '28px', height: `${heightPercent}px`, background: `linear-gradient(180deg, ${colors.chartBar} 0%, ${colors.chartBar}80 100%)`, borderRadius: '6px 6px 0 0', transition: 'height 0.5s ease-out' }} />
                                            <span style={{ fontSize: '10px', color: colors.hint, marginTop: '8px', transform: 'rotate(-45deg)', whiteSpace: 'nowrap' }}>{shortDate}</span>
                                        </div>
                                    );
                                }) : (
                                    <div style={{ width: '100%', textAlign: 'center', color: colors.hint, alignSelf: 'center' }}>{tr('insights.no_data', lang)}</div>
                                )}
                            </div>
                        </div>

                        {/* Top sources */}
                        <div style={{ background: colors.secondaryBg, padding: '20px 16px', borderRadius: '16px', border: `1px solid ${colors.hint}20` }}>
                                    ...
                                    {topSources.map((source, index) => (
                                        <div key={index} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: index < 3 ? colors.chartBar : `${colors.hint}40`, color: index < 3 ? '#fff' : colors.text, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 'bold' }}>
                                                    {index + 1}
                                                </div>
                                                <span style={{ fontSize: '14px', fontWeight: '500' }}>{source.name}</span>
                                            </div>
                                            <span style={{ fontSize: '13px', color: colors.hint, fontWeight: '600' }}>
                                                {source.subscriberCount} <span style={{ fontSize: '11px' }}>{tr('insights.users', lang)}</span>
                                            </span>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                <div style={{ textAlign: 'center', color: colors.hint }}>{tr('insights.no_sources', lang)}</div>
                            )}
                        </div>
                    </div>
                )}
